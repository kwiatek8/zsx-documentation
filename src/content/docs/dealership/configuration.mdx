...
<Code code="xclusive.Retrieval = {
    use = true, -- SET USE TO TRUE 
    type = 'COINS', 
}" lang="lua" ins="true"></Code>
...
Editable.ExclusivePayForVehicle = function(src, vehData)
    ...
    return receivedPay
    -- For help contact on discord discord.gg/MF9uVuWrQn
end", lang="lua" ins=" --Apply the code check if user have exclusive currency required for vehicle price. Example  --removeCoinsFunc(vehData.price)">
